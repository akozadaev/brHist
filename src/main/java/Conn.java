...
public class Conn {
    ...
    private static List<String> getTableCollNames(Statement statmt, String tableName){
    ...
    private static List<String> getAllTableNamesInDb(Statement statmt){
        List<String> result = new LinkedList<String>();
        try {
            resSet = statmt.executeQuery("select name from sqlite_master\n" +
                    "where type = 'table'");
            while(resSet.next()){
                result.add(resSet.getString("name"));
            }
    ...
    private static List<String> ReadDBRows(Statement statmt, String tableName) throws ClassNotFoundException, SQLException {
        List<String> result = new LinkedList<String>();
    ...
    public static void ReadDB() throws ClassNotFoundException, SQLException
    {
        ...
        for (String dbName: dbs){
            ...
            try{
                FileWriter writer = new FileWriter("results/" +dbName + ".csv", false);
                for (String s: dbRows){
                    writer.write(s);
                    // запись по символам
                    writer.append('\n');
